...
from helpers import (
    get_logger, check_env, blockfrost_get, koios_get, pg_connect,
...
# Rows accumulated before a PostgreSQL bulk upsert.
...
def sync_drep_info(logger=None, dreps_per_run=DREPS_PER_RUN, should_cancel=None):
    ...
    logger.info("=== Sync: drep_info (Blockfrost → PostgreSQL, incremental) ===")
    ...
    # 2. Load checkpoint + full drep_id list from PostgreSQL drep_list.
    conn = pg_connect()
    try:
    ...
    # 3. Fresh connection per flush: PgBouncer (transaction pooling)
    #    closes idle connections mid-run.
    def upsert_pg(batch_rows):
        for attempt in range(3):
            c = None
            try:
                c = pg_connect()
                pg_upsert_batch(c, "drep_info", columns, batch_rows,
                                conflict_cols=["drep_id"])
                return
            except Exception as e:
                if attempt == 2:
                    raise
                logger.warning(f"[drep_info] PostgreSQL retry {attempt + 1}: {e}")
    ...
    try:
        c = pg_connect()
        count = pg_row_count(c, "drep_info")
        logger.info(f"[drep_info] Row count: {count}")
    except Exception as e:
        logger.warning(f"[drep_info] Count failed: {e}")
    finally:
        if c is not None:
            try:
                c.close()
