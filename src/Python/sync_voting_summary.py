...
from helpers import (
    get_logger, check_env, koios_get, pg_connect,
    pg_upsert_batch, pg_row_count, now_iso, gen_uuid, pg_query,
)
...
def sync_voting_summary(logger=None):
    ...
    logger.info("=== Sync: proposal_voting_summary (Koios → PostgreSQL) ===")
    ...
    conn.close()
    proposal_ids = [r[0] for r in prop_res]
    # Skip test proposals
    ...
    for i, pid in enumerate(proposal_ids):
        try:
            ...
            if len(all_rows) >= BATCH_SIZE:
                try:
                    pg_upsert_batch(conn, "proposal_voting_summary", columns, all_rows,
                                    conflict_cols=["proposal_id"], preserve_cols=["id", "created_at"])
                except Exception as e:
                    logger.error(f"[voting_summary] Insert error: {e}")
                all_rows = []
    ...
    if all_rows:
        try:
            ...
            logger.error(f"[voting_summary] Final insert error: {e}")

    logger.info(f"\n[voting_summary] Done, errors={errors}")
    count = pg_row_count(conn, "proposal_voting_summary")
    logger.info(f"[voting_summary] Row count: {count}")
    conn.close()
    return count
